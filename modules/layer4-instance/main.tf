locals {
  ...
  cross_zone_enabled = length(var.availability_zones) > 1
...
resource "tencentcloud_clb_instance" "this" {
  ...
  master_zone_id  = local.cross_zone_enabled ? var.availability_zones[0] : null
  slave_zone_id   = local.cross_zone_enabled ? var.availability_zones[1] : null

  ## Logging
  log_set_id   = var.access_log.enabled ? module.access_log[0].log_set_id : null
  log_topic_id = var.access_log.enabled ? module.access_log[0].log_topic_id : null
  ...
  sla_type = var.sla.enabled ? local.sla_type[var.sla.type] : null
